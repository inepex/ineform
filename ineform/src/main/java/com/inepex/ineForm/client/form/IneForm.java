...
public class IneForm implements DisplayedFormUnitChangeHandler {
    ...
    private Multimap<String, KeyValueObjectValidator> customValidators = ArrayListMultimap.create();
    ...
    private void process(PanelWidget parentPanel, List<Node<FormRDescBase>> nodesToProcess) {
     ...
     * for root
     */
    private PanelWidget createPanelAndAddToParent(List<Node<FormRDescBase>> children) {
        PanelWidget panel = formCtx.panelWidgetFactory.createPanel(null, null, this);
    ...
    private
        void
        createFormAndAddToParent(PanelWidget parentPanel, Node<FormRDescBase> formdescNode) {
    ...
    public HandlerRegistration addBeforeRenderHandler(BeforeRenderEvent.Handler handler) {
    ...
    public HandlerRegistration addFilledWithDataHandler(FilledWithDataEvent.Handler handler) {
    ...
    public void dealValidationResult(ValidationResult vr) {
        ...
        for (AbstractFormUnit unit : getRootPanelWidget().getFormUnits()) {
            ...
            for (String modelKey : unit.getFormWidgetKeySet()) {
                unit.getWidgetByKey(modelKey).removeStyleName(
                    ResourceHelper.ineformRes().style().formWidgetError());
            }
        }
            ...
            for (AbstractFormUnit unit : getRootPanelWidget().getFormUnits()) {
                ...
                for (String modelKey : unit.getFormWidgetKeySet()) {
                    if (vr.getFieldErrors().containsKey(modelKey))
                        unit.getWidgetByKey(modelKey).addStyleName(
                            ResourceHelper.ineformRes().style().formWidgetError());
                }
            }
        }
        ...
        if (vr != null && !vr.isValid() && vr.getFieldErrors() != null) {
            Set<String> unTargettedKeys = new HashSet<String>(vr.getFieldErrors().keySet());
            for (AbstractFormUnit unit : getRootPanelWidget().getFormUnits()) {
    ...
    /**
     * Important: Be aware of the differences to setData(...) see javadoc of
     * setData(...)
     * 
     * @param data
     */
    public void setInitialData(AssistedObject data) {
        ...
        if (data.getDescriptorName() == null || !descriptorName.equals(data.getDescriptorName()))
        ...
        for (AbstractFormUnit form : getRootPanelWidget().getFormUnits()) {
            for (String key : form.getFormWidgetKeySet()) {
                ...
                try {
                    ...
                    System.out.println("Descriptor: " + descriptorName + " Key: " + key);
                    e.printStackTrace();
                }
            }
        }
    ...
    /**
     * Important: Be aware of the differences to setInitialData(...) When using
     * setData() the difference produced from the form when saving will contain
     * these values, too!
     * 
     * @param data
     */
    ...
    public
        AssistedObject
        getValuesOfFormUnit(AssistedObject objectToFill, AbstractFormUnit formUnit) {
        ...
        for (String key : formUnit.getFormWidgetKeySet()) {
            ...
            try {
                switch (type) {
                    case BOOLEAN:
                        actual.set(lastKey, widget.getBooleanValue());
                        break;
                    case DOUBLE:
                        actual.set(lastKey, widget.getDoubleValue());
                        break;
                    case LIST:
                    ...
                    case LONG:
                        ...
                        break;
                    case RELATION:
                        actual.set(lastKey, widget.getRelationValue());
                        break;
                    case STRING:
                        actual.set(lastKey, widget.getStringValue());
                        break;
                    case PROP:
                        actual.getAssistedObject().setPropsJson(
                            ((PropFDesc) widget.getFieldDescriptor()).getId(),
    ...
    /**
     * 
     * reset values to empty kvo clear validation result
     * 
     */
    ...
    /**
     * 
     * reset values to initial data (or empty kvo) clear validation result
     * 
     */
     ...
     * 
     * @param vr
     *            - set invalid if relation list contains error!!!!
     * @param fields
     *            - null = all
     */
    private void validateRelationLists(ValidationResult vr, Collection<String> fields) {
        ...
        for (AbstractFormUnit unit : getRootPanelWidget().getFormUnits()) {
            for (String s : unit.getFormWidgetKeySet()) {
                if (!(unit.getWidgetByKey(s) instanceof RelationListFW)
                    || (fields != null && !fields.contains(s)))
                    continue;
