...
public class PropFWReadOnlyView extends HandlerAwareComposite {

    private final Grid grid = new Grid();

    private List<PropRow> rows = new ArrayList<PropRow>();
    private boolean showHeader = true;
    private boolean showType = false;
    private boolean rendered = false;

    @Inject
    public PropFWReadOnlyView() {
        initWidget(grid);
    }
    ...
    public void init(List<PropRow> rows, boolean showHeader, boolean showType) {
        this.rows = rows;
    ...
    private void createHeader() {
        ...
        grid.getCellFormatter().setStyleName(
            0,
            0,
            ResourceHelper.ineformRes().style().customKVOHeaderRO());
        if (showType) {
            grid.getCellFormatter().setStyleName(
                0,
                1,
                ResourceHelper.ineformRes().style().customKVOHeaderTypeRO());
            grid.getCellFormatter().setStyleName(
                0,
                2,
                ResourceHelper.ineformRes().style().customKVOHeaderRO());
    ...
    private void showRow(int rowNr, PropRow row) {
        grid.setText(rowNr, 0, row.getKey());
        if (showType) {
            grid.setText(rowNr, 1, PropFieldType.getName(row.getType()));
