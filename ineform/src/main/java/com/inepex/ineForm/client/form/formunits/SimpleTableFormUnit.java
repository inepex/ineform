...
/**
 * A simple form, that uses HTML table as base layout. The table has four
 * columns, one for description,one for mandatory sign, one for the form widgets
 * and one for validation error
 * 
 * @author istvan
 *
 */
public class SimpleTableFormUnit extends AbstractFormUnit {
    ...
    /**
     * Renders the widgets to the form. Either using the default
     * {@link FormRDesc} or a custom one if provided by
     * {@link #setCustomRenderDescriptor(FormRDesc)}
     */
    protected void renderForm(List<Node<FormRDescBase>> selectedNodes, FormRDesc formRDesc) {
        ...
        for (int i = 0; i < selectedNodes.size(); i++) {
            ...
            if (!nodeElement.hasProp(putNextWidgetIntoThisLineToo)) {
                ...
                if (widgetsInThisLine.size() > 0) {
                    ...
                    if (mandatory && displayTitle)
                        mainTable.setText(row, 1, "*");
        ...
        if (widgetsInThisLine.size() > 0)
            throw new RuntimeException(
                "SimpleTableFormUnit: can't set 'putNextWidgetIntoThisLineToo' on the last line");
        // correct size
