...
class DateTimeTextBox extends HandlerAwareComposite {
    ...
    public void manageSelectSimple() {
        ...
        if (b > 0)
            b--;
        while (b > 0 && value.charAt(b) >= '0' && value.charAt(b) <= '9')
            b--;
        if (b > 0 || value.charAt(b) < '0' || value.charAt(b) > '9')
            b++;
        ...
        if (e > value.length() - 1) {
            ...
            while (e < value.length() - 1 && value.charAt(e) >= '0' && value.charAt(e) <= '9')
                e++;
            if (e < value.length() - 1 || value.charAt(e) < '0' || value.charAt(e) > '9')
                e--;
        }

        if (b <= e)
            wrappedtb.setSelectionRange(b, e - b + 1);
    ...
    public void manageSelectOnArrow() {
        ...
        while (cp > 0 && txt.charAt(cp) <= '9' && txt.charAt(cp) >= '0')
            cp--;
    ...
    public static class DateTimeTextBoxEvent extends GwtEvent<DateTimeTextBoxEventHandler> {
        ...
        @Override
        public
            com.google.gwt.event.shared.GwtEvent.Type<DateTimeTextBoxEventHandler>
            getAssociatedType() {
            return TYPE;
        }
    }
