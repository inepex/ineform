...
public class TestDataCreator {
	...
	private static void createCoreModuleRows() {
		...
		
		{
			...
			row.setModule(coreModule);
			coreModule.getRows().add(row);
			row.getValues().add(devTranVal(row, en, "<b>System statistic:</b><br/>{statTable}<br/>" +
		...
		
		{
			...
			row.setModule(coreModule);
			coreModule.getRows().add(row);
			row.getValues().add(devTranVal(row, en, "Invalid usage of {function}. See details at {docReference}", dev, 0));
		}
	...
	private static TranslatedValue devTranVal(ModuleRow row, Lang lang, String value, User u, long creationMinus) {
