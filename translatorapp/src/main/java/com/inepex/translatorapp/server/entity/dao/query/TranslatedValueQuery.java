...
public class TranslatedValueQuery extends BaseQuery<TranslatedValue>{
	...
	public Expression<Boolean> buildWhere(
		...
		TranslatedValueSearchHandler handler = handlerFactory.createSearchHandler(action.getSearchParameters());
		...
		if (lang!=null)
			base = addAndExpression(cb, base, cb.equal(from.get(TranslatedValue_.lang).get(Lang_.id), lang.getId()));
		Relation row = handler.getRelation(TranslatedValueConsts.s_row);
		if (row!=null)
			base = addAndExpression(cb, base, cb.equal(from.get(TranslatedValue_.row).get(ModuleRow_.id), row.getId()));
	return base;
	}
	...
	public Order getOrderExpression(
		...
		if(idList.get(0).equals(TranslatedValueConsts.k_lang)){
			if(idList.size()==1) {
		...
		if(idList.get(0).equals(TranslatedValueConsts.k_row)){
			if(idList.size()==1) {
