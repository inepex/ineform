...
public class ModuleRowQuery extends BaseQuery<ModuleRow> {
    ...
    public Expression<Boolean> buildWhere(
        ...
        ModuleRowSearchHandler handler =
        ...
        if (key != null)
            base =
                addAndExpression(
                    cb,
                    base,
                    cb.like(cb.upper(from.get(ModuleRow_.key)), key.toUpperCase() + "%"));
        ...
        if (module != null)
            base =
                addAndExpression(
                    cb,
                    base,
                    cb.equal(from.get(ModuleRow_.module).get(Module_.id), module.getId()));
        return base;
    }
    ...
    public
        Order
        getOrderExpression(AbstractSearch action, CriteriaBuilder cb, Root<ModuleRow> from) {
        Order o;
