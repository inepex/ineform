...
public class TranslatedValueDao extends BaseDao<TranslatedValue> {
    ...
    public CriteriaSelector<TranslatedValue, TranslatedValue> getSelector() {
        return new CriteriaSelector<TranslatedValue, TranslatedValue>(
            em,
            getQuery(),
    ...
    public CriteriaSelector<Long, TranslatedValue> getCountSelector() {
        return new CriteriaSelector<Long, TranslatedValue>(
            em,
            getQuery(),
            Long.class,
            TranslatedValue.class);
    ...
    public List<TranslatedValue> listForTranslatorPage(
        boolean data,
        List<Long> userLangs,
        Integer firstResult,
        Integer maxResult,
        String moduleName,
        TranslateListingType listType) {
        ...
        switch (listType) {
            ...
            case Outdated:
                ...
                query
                    .append("not exists (select 1 from TranslatedValue tv2 where tv2.lang.isoName = '");
                query.append(Consts.defaultLang);
                query.append("' ");
                query.append("and tv2.row.id = tv.row.id) ");

                query
                    .append("or exists (select 1 from TranslatedValue tv2 where tv2.lang.isoName = '");
                query.append(Consts.defaultLang);
                query.append("' ");
