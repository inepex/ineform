...
public class LangQuery extends BaseQuery<Lang> {
        ...
        if (isoName != null)
            base =
                addAndExpression(
                    cb,
                    base,
                    cb.like(cb.upper(from.get(Lang_.isoName)), isoName.toUpperCase() + "%"));
    ...
    public
        Expression<Boolean>
        getSearchExpression(CriteriaBuilder cb, Path<Lang> from, String value) {
