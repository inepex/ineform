...
@Singleton
public class TranslateTableRowHandlerFactory
    implements
    HandlerFactoryI<TranslateTableRowHandlerFactory.TranslateTableRowHandler> {

    private final DescriptorStore descriptorStore;

    /**
     * can be injected or created with 'new
     * TranslateTableRowHandlerFactory(decStore)'
     */
    @Inject
    ...
    public TranslateTableRowHandler createHandler(AssistedObject assistedObject) {
        if (!TranslateTableRowConsts.descriptorName.equals(assistedObject.getDescriptorName()))
            throw new IllegalArgumentException("Type incompatibility: handler: '"
                + TranslateTableRowConsts.descriptorName
                + "' assistedObject: '"
    ...
    public TranslateTableRowSearchHandler createSearchHandler() {
        AssistedObject assistedObject =
            new KeyValueObject(TranslateTableRowConsts.searchDescriptor);
    ...
    public TranslateTableRowSearchHandler createSearchHandler(AssistedObject assistedObject) {
        if (!TranslateTableRowConsts.searchDescriptor.equals(assistedObject.getDescriptorName()))
            throw new IllegalArgumentException("Type incompatibility: handler: '"
                + TranslateTableRowConsts.searchDescriptor
                + "' assistedObject: '"
                + assistedObject.getDescriptorName()
                + "'");
    ...
    public static class TranslateTableRowHandler extends AssistedObjectHandler {
        ...
        private TranslateTableRowHandler(
            AssistedObject assistedObject,
    ...
    public static class TranslateTableRowSearchHandler extends AssistedObjectHandler {
        ...
        private TranslateTableRowSearchHandler(
            AssistedObject assistedObject,
            DescriptorStore descriptorStore) {
            super(assistedObject, descriptorStore);
        }
