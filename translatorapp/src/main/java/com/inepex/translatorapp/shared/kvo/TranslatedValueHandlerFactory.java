...
@Singleton
public class TranslatedValueHandlerFactory
    implements
    HandlerFactoryI<TranslatedValueHandlerFactory.TranslatedValueHandler> {

    private final DescriptorStore descriptorStore;

    /**
     * can be injected or created with 'new
     * TranslatedValueHandlerFactory(decStore)'
     */
    @Inject
    ...
    public TranslatedValueSearchHandler createSearchHandler(AssistedObject assistedObject) {
        if (!TranslatedValueConsts.searchDescriptor.equals(assistedObject.getDescriptorName()))
            throw new IllegalArgumentException("Type incompatibility: handler: '"
                + TranslatedValueConsts.searchDescriptor
                + "' assistedObject: '"
                + assistedObject.getDescriptorName()
                + "'");
    ...
    public static class TranslatedValueHandler extends AssistedObjectHandler {
        ...
        private TranslatedValueHandler(
    ...
    public static class TranslatedValueSearchHandler extends AssistedObjectHandler {
        ...
        private TranslatedValueSearchHandler(
            AssistedObject assistedObject,
            DescriptorStore descriptorStore) {
            super(assistedObject, descriptorStore);
        }
