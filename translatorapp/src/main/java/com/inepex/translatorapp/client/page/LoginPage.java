...
public class LoginPage extends FlowPanelBasedPage {
    ...
    LoginPage(
        ...
        mainPanel.add(new TranslatorLoginBox(
            authManager,
            historyProvider,
            eventBus,
            ineDispatch,
        ...
        protected void doLoggedinLogic(AuthStatusResultBase base) {
            ...
            if (authManager.getLastAuthStatusResult().getRoles().contains(Consts.Roles.developer)
                || authManager
                    ...
                    .getRoles()
                    .contains(Consts.Roles.translator))
                eventBus.fireEvent(new PlaceRequestEvent(AppPlaceHierarchyProvider.LOGGEDIN));
            else
                eventBus.fireEvent(new PlaceRequestEvent(new PlaceToken(
                    AppPlaceHierarchyProvider.LOGGEDIN).appendChild(
                    AppPlaceHierarchyProvider.INACTIVE).toString()));
        }
    ...
    // HistoryProvider historyProvider, EventBus eventBus, IneDispatch
    // ineDispatch) {
    ...
    // PlaceRequestEvent(AppPlaceHierarchyProvider.LOGGEDIN));
    // else
