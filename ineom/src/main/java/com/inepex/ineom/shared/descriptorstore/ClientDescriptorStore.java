...
/**
 * This class is used for storing {@link ObjectDesc}s, {@link FormRDesc}s,
 * {@link ValidatorDesc}s {@link TableRDesc}s by descriptorName
 * 
 * RenderDescriptors can have specific names, although to all ObjectDescriptors
 * a default instants from all 3 types of descriptors should exist.
 * 
 * @author istvan
 *
 */
public class ClientDescriptorStore extends DescriptorStore {
    ...
    public FDesc getRelatedFieldDescrMultiLevel(ObjectDesc baseOD, List<String> path) {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            RelationFDesc relFieldDescr = (RelationFDesc) relObjectDesc.getField(path.get(i));
