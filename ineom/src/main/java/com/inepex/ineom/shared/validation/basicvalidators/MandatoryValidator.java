...
public class MandatoryValidator implements KeyValueObjectValidator {
    ...
    public void doValidation(AssistedObject kvo, ValidationResult validationResult) {
        ...
        switch (type) {
            case BOOLEAN:
                if (checker.getBoolean(fieldName) == null)
                    msg = IneOmI18n.validator_mandatory();
                break;
            case DOUBLE:
                if (checker.getDouble(fieldName) == null)
                    msg = IneOmI18n.validator_mandatory();
                break;
            case LIST:
                if (checker.getList(fieldName) == null)
                    msg = IneOmI18n.validator_mandatory();
                break;
            case RELATION:
                if (checker.getRelation(fieldName) == null)
                    msg = IneOmI18n.validator_mandatory();
                break;
            case LONG:
                if (checker.getLong(fieldName) == null)
                    msg = IneOmI18n.validator_mandatory();
                break;
            case STRING:
                if (checker.getString(fieldName) == null)
                    msg = IneOmI18n.validator_mandatory();
                break;
        }
