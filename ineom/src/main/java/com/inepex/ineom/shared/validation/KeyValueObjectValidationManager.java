...
public class KeyValueObjectValidationManager {
    ...
    @Inject
    protected KeyValueObjectValidationManager(
     ...
     * @return
     */
    public KeyValueObjectValidator createBaseValidator(
        IneT type,
        String fieldName,
        ...
        if (validatorName.contains(BEFORE))
            return new BeforeAfterValidator(
                fieldName,
                validatorName.replaceAll(BEFORE + ":", ""),
                false,
                fieldDisplayname,
                od);
        if (validatorName.contains(AFTER))
            return new BeforeAfterValidator(
                validatorName.replaceAll(AFTER + ":", ""),
                fieldName,
                true,
                fieldDisplayname,
                od);
        ...
        if (validatorName.contains(MAXLENGTH))
            return new LengthValidator(
                ...
                fieldName,
                Type.MAXLENGTH,
                Integer.parseInt(validatorName.replaceAll(MAXLENGTH + ":", "")),
                od);
        ...
        if (validatorName.contains(MINLENGTH))
            return new LengthValidator(
                ...
                fieldName,
                Type.MINLENGTH,
                Integer.parseInt(validatorName.replaceAll(MINLENGTH + ":", "")),
                od);
        ...
        if (validatorName.equals(ALPHANUM))
            return new RegexprValidator(
                type,
                fieldName,
                od,
                "[\\w]*",
                "[\\w]*",
                IneOmI18n.validationAlphanum());
        if (validatorName.equals(ALPHANUM_OR_SPACE))
            return new RegexprValidator(
                ...
                "[\\w ]*",
                "[\\w ]*",
                IneOmI18n.validationAlphanumOrSpace());
    ...
    public ValidationResult validatePartial(
        ...
        return validatePartial(
            kvo,
            fieldNames,
            descStore.getOD(kvo.getDescriptorName()),
            customValidators);
    }

    /**
     * 
     * @param extravalidators
     *            - never will check extravalidators' getUsedFields, just they
     *            will be ran
     * @return
     */
    public ValidationResult validatePartial(
        ...
        for (String fieldName : fieldNames) {
            ...
            // TODO relation list: there is some rel list validation in
            // SaveCancelForm
        }
