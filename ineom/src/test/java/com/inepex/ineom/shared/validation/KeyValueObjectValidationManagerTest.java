...
public class KeyValueObjectValidationManagerTest {
	...
	public void init() {
		...
		store.registerModule(new IneOmI18n(new ServerIneOmI18nProvider(new TestCurrentLangProvider())));
	...
	public void testBase() {
		...
		assertVR(validationManager.validate(createKvo("1234567890_1234123", "alma@alma.hu")),false, true);
	...
	private void assertVR(ValidationResult vr, boolean f1Valid, boolean f2Valid) {
		...
		assertEquals(
				(f1Valid ? 0 : 1)+
				(f2Valid ? 0 : 1), vr.getFieldErrors().size());
		if(!f1Valid)
