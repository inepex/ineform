...
public class KeyValueObjectSerializerTest {
    ...
    public void defaultSerialize() {
        String serialized =
            new KeyValueObjectSerializer(TestUtil.getTestKvo(), "{sep}", "=").serializeToString();
    ...
    public void customListSerializer() {
        String serialized =
            new KeyValueObjectSerializer(TestUtil.getTestKvo(), "{sep}", "=").setListSerializer(
                new ListSerializer() {
                    ...
                    public String serialize(IneList list) {
                        ...
                        for (Relation rel : list.getRelationList()) {
                            AssistedObjectHandler handler =
                                TestUtil.createObjectHandlerFactory().createHandler(rel.getKvo());
    ...
    public void customRelationSerializer() {
        String serialized =
            new KeyValueObjectSerializer(TestUtil.getTestKvo(), "{sep}", "=")
                .setRelationSerializer(new RelationSerializer() {

                    @Override
                    public String serialize(Relation relation) {
                        AssistedObjectHandler handler =
                            TestUtil.createObjectHandlerFactory().createHandler(relation.getKvo());
