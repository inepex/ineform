...
public class GreenScrollPanel implements HasScrolling, IsWidget {
    ...
    private static boolean hasMacNanoScrollBar() {
        ...
        return (has(lowerCaseAgent, "chrome") || has(lowerCaseAgent, "safari"))
            ...
            && doesntHave(lowerCaseAgent, "mac os x 10_5")
            && doesntHave(lowerCaseAgent, "mac os x 10_6");
    }
    ...
    protected static class GreenScrollImpl extends CustomScrollPanel {
        ...
        public GreenScrollImpl(boolean withHorizontalScroll) {
            ...
            if (!withHorizontalScroll) { // important hack to avoid random
                                         // scrollbars
        ...
        protected void onAttach() {
            ...
            // hack to hide scrollbar in chrome when leaflet map is also (check
            // GreenScrollPanel.onAttach too)
            // displayed
            if (isChrome()) {
                try {
                    ((Element) (getElement().getChild(1).getChild(0).getChild(0).getChild(0)))
                        .getStyle()
                        .setZIndex(-1000);
                    ((Element) (getElement().getChild(1).getChild(0).getChild(0).getChild(1)))
                        .getStyle()
                        .setZIndex(-1000);
                } catch (Exception e) {
                    GWT.log("", e);
