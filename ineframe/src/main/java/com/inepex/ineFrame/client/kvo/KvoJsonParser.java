...
public class KvoJsonParser {
    ...
    public KvoJsonParser setCustomResultExtractors(
        Map<String, ResultObjectExtractor> customResultExtractors) {
        this.customResultExtractors = customResultExtractors;
        return this;
    }

    public AssistedObject parse() throws RuntimeException {
        AssistedObjectHandler kvoChecker = factory.createHandler(new KeyValueObject(od.getName()));
        ...
        for (String key : od.getKeys()) {
            if (jso.containsKey(key)) {
                ...
                switch (fdesc.getType()) {
                    case BOOLEAN:
                        if (childJso.isNull() != null)
                            kvoChecker.set(key, (Boolean) null);
                    ...
                    case DOUBLE:
                        if (childJso.isNull() != null)
                            kvoChecker.set(key, (Double) null);
                    ...
                    case STRING:
                        if (childJso.isNull() != null)
                            kvoChecker.set(key, (String) null);
                    ...
                    case RELATION:
                        if (childJso.isNull() != null)
                            kvoChecker.set(key, (Relation) null);
                    ...
                    case LIST:
                        if (childJso.isNull() != null)
                            kvoChecker.set(key, (IneList) null);
