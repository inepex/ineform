...
public class PlaceHandlerHelper {
    ...
    public static String appendParam(
        String hierarchicalToken,
        String paramToken,
        List<? extends Object> listValue) {
        String value;
    ...
    public static String removeParam(String hierarchicalToken, String paramName) {
        ...
        if (paramStart != -1) {
            ...
            if (nextControl == -1)
                return hierarchicalToken.substring(0, paramStart);
            else if (PlaceHandler.AND_SIGN.equals("" + hierarchicalToken.charAt(nextControl)))
                return hierarchicalToken.substring(0, paramStart + 1)
                    + hierarchicalToken.substring(nextControl + 1);
            else
                return hierarchicalToken.substring(0, paramStart)
                    + hierarchicalToken.substring(nextControl);
        } else {
    ...
    public static void updateHierarchicalTokens(String currentFullToken, Node<InePlace> placeRoot) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            for (Node<InePlace> pn : pointer.getChildren()) {
                ...
                pn.getNodeElement().setHierarchicalToken(
                    PlaceHandlerHelper.appendChild(curreFullPart.toString(), pn.getNodeId()));
            }
    ...
    public static int levelOfChange(List<String> lastTokenParts, List<String> newTokenParts) {
        if (lastTokenParts == null)
            return 0;
        if (newTokenParts == null)
            return 0;
        ...
        if (lastTokenParts.size() == newTokenParts.size())
            return -1;
        else
            return minLength;
    ...
    /**
     * Example currentToken:
     * home/devices?deviceGroups=1&chart=false&details=true
     * /device?deviceId=100000/general target: home/devices/device returns:
     * home/
     * devices?deviceGroups=1&chart=false&details=true/device?deviceId=100000
     * 
     */
    public static String findActualLevelWithParams(String currentToken, String target) {
        ...
        if (splittedToken.length() > 0)
            splittedToken = splittedToken.substring(0, splittedToken.length() - 1);
