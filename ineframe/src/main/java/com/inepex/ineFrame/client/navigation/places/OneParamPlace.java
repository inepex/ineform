...
/**
 * Handles one parameter. If parameter set, automatically redirects to
 * defaultChild. Else page can provide redirection.
 * 
 * @author SoTi
 *
 */
public class OneParamPlace extends ParamPlace {
    ...
    public void processParams(
        ...
        final UrlParamsParsedCallback callback) {
        ...
        try {
            if (!PlaceHandlerHelper.getPlacePart(requestedToken).endsWith(placeId)) {
                ...
                if (param == null) {
                    // param not set, can redirect through callback to a default
                    // selection
