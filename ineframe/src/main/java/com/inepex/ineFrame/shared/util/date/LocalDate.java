...
public abstract class LocalDate<T extends LocalDate<T>> extends LocalDateBase {
    ...
    public T addDaysSafe(int days) {
        Date d = date();
        // the Date class handles it correctly even if the month doesn't have
        // that many days as
        // it is set in the argument (it adjusts the year, month and day if
        // necessary)
    ...
    /**
     * Adds month to the current date. If the
     * {@link LocalDate#isLastDayOfMonth(Date)} is true before adding, it will
     * be true after it. For example {@code <Apr 30>+1 month will be <May 31>}
     */
    @SuppressWarnings({ "deprecation", "unchecked" })
