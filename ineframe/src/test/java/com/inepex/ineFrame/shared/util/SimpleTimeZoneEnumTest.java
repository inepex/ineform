...
//	@Test
	public void ceckTimeZones() {
		...
		for(SimpleTimeZoneEnum tz : SimpleTimeZoneEnum.values()) {
			...
			Assert.assertEquals("Invalid offset for: "+tz.displayName, javaOffsetInMin, gwtConstOffset);
			
			String pref = namePrefixFromOffset(javaOffsetInMin);
			Assert.assertEquals("Invalid displ name: "+tz.displayName, pref, tz.displayName.substring(0, tz.displayName.indexOf(")")+1));
		}
	}
...
//	@Test
	public void testGetGwtconstOffsetInMin() {
		Assert.assertEquals(120, getGwtconstOffsetInMin("{\"transitions\": [], \"names\": [\"CAT\", \"Central Africa Time\"], \"id\": \"Africa/Harare\", \"std_offset\": 120}"));
		Assert.assertEquals(-120, getGwtconstOffsetInMin("{\"transitions\": [], \"names\": [\"ex\"], \"id\": \"Ex\", \"std_offset\": -120}"));
	}
