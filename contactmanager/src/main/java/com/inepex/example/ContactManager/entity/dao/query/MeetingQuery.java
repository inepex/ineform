...
public class MeetingQuery extends BaseQuery<Meeting> {
    ...
    @Override
    public Order getOrderExpression(AbstractSearch action, CriteriaBuilder cb, Root<Meeting> from) {
        Order o;
        ...
        } else if (idList.get(0).equals(MeetingConsts.k_company)) {
            if (idList.size() == 1) {
        ...
        } else if (idList.get(0).equals(MeetingConsts.k_contact)) {
            if (idList.size() == 1) {
